	...
	public function _get_chat_messages()
	{
		...
		if($messages->num_rows() > 0){
			...
			foreach($messages->result() as $message)
			{
				...
				{
					$unread = '<img alt="image" class="img-circle" src="'.base_url().'/public/user_img/'.$img.'">&nbsp;<strong>'.$message->username.'</strong><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
					$unread .='<strong>'.$message->chat_message_content.'</strong><br>';
				}
				else
				{
            ...
            <small class="pull-right">'.$this->_time_ago($message->date_created).'</small>'.$unread.'                                                             
                ...
                $chat_html.='<li class="divider" ></li></li>';
                   
                        
			}
			

			$chat_html .='<li style ="list-style-type:none; text-transform:none;">
		...
		if($alert->num_rows() > 0)
		{
			...
			foreach ($alert->result() as $notif) {
				...
				if ($notif->type == "follow"){
					$type = '<img alt="image" class="img-circle" width=38px height=38px src="'.base_url().'/public/user_img/'.$img.'"><i class="fa fa-link fa-fw"></i>';
				...
				elseif($notif->type == "like"){
					$type = '<img alt="image" class="img-circle" width=38px height=38px src="'.base_url().'/public/user_img/'.$img.'"><i class="fa fa-thumbs-up fa-fw"></i>';
					$type .=''.$notif->username.' is liked you';
					$type .='<span class="pull-right text-muted small">'.$this->_time_ago($notif->date_created).'</span>';
				}


				$noti_html.='<li style ="list-style-type:none; text-transform:none;">';
